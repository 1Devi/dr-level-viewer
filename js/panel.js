...
export function renderInfo() {
  ...
  $("meta").innerHTML =
    ...
    q("zoom", '<span title="' + lv.zoom.x + " / " + lv.zoom.y + '">' + +lv.zoom.x.toFixed(5) + " / " + +lv.zoom.y.toFixed(5) + "</span>") +
  ...
  $("objs").innerHTML =
    used
      .map(
        (d) =>
          '<button class="grow' +
          (state.hidden.has("o|" + d.id) ? " off" : "") +
          '" data-g="o|' +
          d.id +
          '">' +
          (d.circle ? '<span class="sw" style="border-radius:50%;background:' + d.circle + '"></span>' : '<img class="sw" style="border:0" src="' + SPR[d.spr] + '" alt="">') +
          '<span class="lbl">' +
          d.label +
          "</span>" +
          '<span class="n">' +
          lv.objects[d.id].length +
          "</span>" +
          "</button>",
      )
...
export function renderPick() {
  ...
  if (!hover) {
    el.innerHTML = '<span class="muted">hover a line or object</span>';
    return;
  }
  if (hover.def) {
    const { def, o, i } = hover;
    const rows = ["<dt>type</dt><dd>" + def.label + " · " + def.hint + "</dd>", "<dt>index</dt><dd>" + (i + 1) + " of " + state.lv.objects[def.id].length + "</dd>", "<dt>position</dt><dd>" + nm(o.x) + ", " + nm(o.y) + "</dd>"];
  ...
  el.innerHTML =
    ...
    l.i +
    " of " +
    state.lv.lines.length +
    "</dd>" +
    "<dt>layer</dt><dd>" +
    (l.solid ? "lines" : "scenery lines") +
    "</dd>" +
    "<dt>from</dt><dd>" +
    nm(l.x1) +
    ", " +
    ...
    "</dd>" +
    "<dt>length</dt><dd>" +
    Math.hypot(l.x2 - l.x1, l.y2 - l.y1).toFixed(1) +
    "</dd>" +
    "<dt>width</dt><dd>" +
    l.th +
    "</dd>" +
    ...
    "</dd>" +
    "</dl>" +
    '<pre class="raw" style="margin-top:8px"><b>' +
    esc(l.src[0]) +
    "</b>\n" +
