...
import {serializeLevel} from "./write.js";
...
let last = null;                   // {recs, px, opt} of the current preview
let timer = 0;

/* ---------- helpers ---------- */
export const isImageFile = f =>
  /^image\//.test(f.type || "") || /\.(png|jpe?g|webp|gif|bmp|avif)$/i.test(f.name || "");
...
const num = (el, a, b, d) => { const v = parseFloat(el.value); return Number.isFinite(v) ? clamp(v, a, b) : d; };
...
async function decode(file){
  ...
  return await new Promise((res, rej) => {
    ...
    im.onerror = () => { URL.revokeObjectURL(url); rej(new Error("could not decode this image")); };
    im.src = url;
  });
}
...
function paint(px, recs, opt){
  const cv = $("genpv"), g = cv.getContext("2d");
  const box = cv.parentElement.getBoundingClientRect();
  const wCss = Math.max(120, Math.floor(box.width));
  const scale = Math.min(wCss/px.W, 260/px.H, 24);
  const dpr = Math.min(window.devicePixelRatio || 1, 2.5);
  const w = Math.max(1, Math.round(px.W*scale)), h = Math.max(1, Math.round(px.H*scale));
...
function refresh(){
  ...
  if(!ready){ $("geninfo").textContent = "choose a PNG or JPG"; return; }
  ...
  paint(px, lv.recs, opt);

  const n = lv.recs.length;
  const rows = 3 + 4*n + 24;
  const kb = Math.round(rows*10/1024);
  $("geninfo").innerHTML =
    px.W + " × " + px.H + " px · <b>" + group(n) + "</b> " + (opt.solid ? "lines" : "scenery") +
    " · " + group(rows) + " rows · ≈" + kb + " KB<br>" +
    "level size " + px.W*opt.size + " × " + px.H*opt.size + " units";
...
const soon = () => { clearTimeout(timer); timer = setTimeout(refresh, 90); };

/* ---------- build ---------- */
function build(){
  if(!last) return null;
  const lv = levelFromPixels(last.px, last.opt);      // fresh records: serialize mutates them
  const out = serializeLevel(lv);
...
export function closeGen(){ $("genwrap").hidden = true; }
...
export function initImageGen(loadFn){
  ...
  $("genwrap").addEventListener("pointerdown", e => { if(e.target === $("genwrap")) closeGen(); });
  addEventListener("keydown", e => { if(e.key === "Escape" && genOpen()) closeGen(); });
  ...
  $("o_wr").addEventListener("input", () => { $("o_w").value = $("o_wr").value; soon(); });
  $("o_w").addEventListener("input", () => { $("o_wr").value = clamp(parseFloat($("o_w").value)||2, 2, MAX_W); soon(); });
  $("o_name").addEventListener("input", () => { $("o_name").dataset.touched = "1"; });
  for(const id of ["o_s","o_layer","o_a","o_p","o_bg","o_skip","o_key","o_tol","o_smooth","o_merge"])
    $(id).addEventListener("input", soon);
  ...
  $("genpv").addEventListener("click", e => {
    ...
    const i = (gy*last.px.W + gx)*4, d = last.px.data;
  ...
  $("gendl").addEventListener("click", () => {
    const b = build();
    if(!b) return;
    const url = URL.createObjectURL(new Blob([b.text], {type: "text/plain"}));
  ...
  addEventListener("resize", () => { if(genOpen()) soon(); });
}
